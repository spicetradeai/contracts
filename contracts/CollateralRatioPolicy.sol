...
interface ITreasury {
    ...
    function totalNetDollarMinted() external view returns (uint256);
...
contract CollateralRatioPolicy is Ownable, ICollateralRatioPolicy, Initializable {
    ...
    function calcEffectiveCollateralRatio() public view returns (uint256) {
        ...
        uint256 total_collateral_value = ITreasury(treasury).globalCollateralValue(); // 285555839726000000
        uint256 net_dollar_minted = ITreasury(treasury).totalNetDollarMinted(); // 347708625625000000

        uint256 ecr = 1000000;
        if (net_dollar_minted > 0) {
          ecr = total_collateral_value.mul(PRICE_PRECISION).div(net_dollar_minted);
        }
    ...
    function refreshCollateralRatio() public {
        ...
        uint256 current_dollar_price = IOracle(oracleDollar).consult(); // 1000041
